...
export default function Admin() {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      if (!appUsersError && appUsersData) {
        ...
        setMessage(`Erro ao carregar usuarios: ${appUsersError.message}`);
      }
  ...
  const handleCreateAppUser = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      setMessage(`Erro ao criar usuario: ${error.message}`);
      return;
    }

    setMessage(`Usuario criado. Senha provisoria: ${tempPassword}`);
    setName('');
    setEmail('');
    setRole('consulta');
    fetchAdminData();
  ...
  const handleToggleStatus = async (userId: string, currentStatus: string) => {
    ...
    if (error) {
      setMessage(`Erro ao atualizar usuario: ${error.message}`);
      return;
    }
  ...
  const handleResetPassword = async (userId: string) => {
    ...
    if (error) {
      setMessage(`Erro ao gerar nova senha: ${error.message}`);
      return;
    }
    setMessage(`Nova senha provisoria: ${tempPassword}`);
    fetchAdminData();
  };
  ...
  const handleDeleteAppUser = async (userId: string) => {
    ...
    if (error) {
      setMessage(`Erro ao remover usuario: ${error.message}`);
      return;
    }
  ...
  return (
    <div className="dashboard-page">
      ...
      <section className="page-card">
        ...
        <p className="page-description">Crie usuarios, gere senha provisoria e copie o convite de acesso.</p>
        <form onSubmit={handleCreateAppUser} style={{ display: 'grid', gap: 14, marginTop: 18 }}>
          <div className="field">
            <label htmlFor="admin-name">Nome</label>
            <input id="admin-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Nome completo" required />
          </div>
          <div className="field">
            <label htmlFor="admin-email">E-mail</label>
            <input id="admin-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="email@exemplo.com" required />
          </div>
          <div className="field">
      ...
      <section className="page-card">
        <div className="recent-header">
          <div>
            <p className="page-label">Usuarios cadastrados</p>
            <h2>Usuarios do App</h2>
          </div>
          <span className="status-pill">{filteredUsers.length} usuario(s)</span>
        </div>

        <div style={{ display: 'flex', gap: 12, marginBottom: 18, flexWrap: 'wrap' }}>
          <input aria-label="Buscar usuario" placeholder="Buscar por nome, e-mail, perfil ou status" value={query} onChange={(event) => setQuery(event.target.value)} style={{ flex: '1 1 260px' }} />
        </div>

        {loading ? (
          <div className="empty-state"><p>Carregando usuarios...</p></div>
        ) : filteredUsers.length === 0 ? (
          <div className="empty-state"><p>Nenhum usuario encontrado.</p></div>
        ) : (
              ...
              <tbody>
                {filteredUsers.map((user) => (
                  <tr key={user.id}>
                    ...
                    <td>{user.name || '-'}</td>
                    <td>{user.role}</td>
                    <td><span className="status-chip">{user.status}</span></td>
                    <td>{user.temporary_password || '-'}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        <button type="button" className="empty-link" onClick={() => handleCopyInvite(user)}>Copiar convite</button>
                        <button type="button" className="empty-link" onClick={() => handleResetPassword(user.id)}>Nova senha</button>
                        <button type="button" className="empty-link" onClick={() => handleToggleStatus(user.id, user.status)}>{user.status === 'ATIVO' ? 'Bloquear' : 'Ativar'}</button>
                        <button type="button" className="empty-link" onClick={() => handleDeleteAppUser(user.id)}>Remover</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <section className="page-card">
        <p className="page-label">Convites antigos</p>
        <h2 style={{ marginTop: 0 }}>Convites registrados</h2>
        {invites.length === 0 ? (
          <div className="empty-state"><p>Nenhum convite antigo encontrado.</p></div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table className="table-list">
              <thead>
