...
import { loadAllPersistentPendingVisits, removePersistentPendingVisit } from '../lib/visitDraft';
...
async function fetchRemoteVisitDetail(id: string) {
  const response = await apiFetch(`/api/visitas?id=${encodeURIComponent(id)}&ts=${Date.now()}`, {
    method: 'GET'
  });
  const payload = await apiReadJson<{ visit?: ApiVisit; error?: string }>(response);
...
async function uploadPhotoToRemoteVisit(visitId: string, photo: SavedPhoto) {
  ...
  const response = await apiFetch(`/api/visitas?action=add-photo&id=${encodeURIComponent(visitId)}&ts=${Date.now()}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
async function synchronizePendingLocals(localVisits: LocalVisitRecord[]) {
  ...
  const persistentPending = await loadAllPersistentPendingVisits();
  const persistentMap = new Map(persistentPending.map((p) => [p.id, p]));

  const mergedLocals = [...localVisits];
  for (const p of persistentPending) {
    if (!mergedLocals.some((m) => m.id === p.id)) {
      mergedLocals.push(p as LocalVisitRecord);
    }
  }

  for (const visit of mergedLocals) {
    ...
    try {
      const persistentItem = persistentMap.get(visit.id);
      const visitToSync = persistentItem || visit;
      const remote = await pushPendingVisit(visitToSync as LocalVisitRecord);
    ...
    <div className="dashboard-page">
      ...
      {editing && isAdminUser && <section className="page-card">
        ...
        <form onSubmit={saveEdit} style={{ display: 'grid', gap: 12 }}>
          ...
          <div className="field">
            <label>Serviços verificados</label>
            <button type="button" className="voice-button" onClick={() => startVoiceInput((t) => updateEdit('servicos', appendDictation(editing.servicos, t)))}>🎤 FALAR SERVIÇOS</button>
            <textarea value={editing.servicos} onChange={(e) => updateEdit('servicos', e.target.value)} />
          </div>
          <div className="field">
            <label>Observações</label>
            <button type="button" className="voice-button" onClick={() => startVoiceInput((t) => updateEdit('observacoes', appendDictation(editing.observacoes, t)))}>🎤 FALAR OBSERVAÇÕES</button>
            <textarea value={editing.observacoes} onChange={(e) => updateEdit('observacoes', e.target.value)} />
          </div>
          <div className="field">
            <label>Conclusão</label>
            <button type="button" className="voice-button" onClick={() => startVoiceInput((t) => updateEdit('conclusao', appendDictation(editing.conclusao, t)))}>🎤 FALAR CONCLUSÃO</button>
            <textarea value={editing.conclusao} onChange={(e) => updateEdit('conclusao', e.target.value)} />
          </div>
