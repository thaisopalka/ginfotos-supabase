...
export default function Visitas({ profile }: VisitasProps) {
  ...
  const loadVisitas = async () => {
    ...
    try {
      ...
      setMessage(`${remoteVisits.length} visita(s) sincronizada(s) carregada(s) para todos os usuários.`);
      setLoading(false);
      return;
    } catch (serverError) {
    ...
    try {
      ...
      if (error) {
        setMessage('Não foi possível carregar visitas sincronizadas. Mostrando visitas salvas neste dispositivo.');
      ...
      setMessage('Tempo esgotado ao carregar visitas. Mostrando visitas salvas neste dispositivo.');
      setVisitas(localVisits);
    } finally {
  ...
  return (
    <div className="dashboard-page">
      <div className="top-row"><div><p className="page-label">Registros</p><h1>Visitas Técnicas</h1></div><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><button type="button" className="empty-button" onClick={loadVisitas}>Atualizar visitas</button><button type="button" className="empty-button" onClick={() => navigate('/nova-visita')}>+ Nova Visita</button></div></div>
